...
async function onNodeSwitch(
  ...
  // 检查权限
  if (!hasAccessByCodes(['Media:PushProxy:List'])) {
    message.error('您没有查看推流代理列表的权限');
    return;
  }
...
const [Grid, gridApi] = useVbenVxeGrid({
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) => {
          ...
          return await getPushProxyPageList(queryParams);
        },
      },
  ...
  } as VxeTableGridOptions<PushProxyApi.PushProxyVO>,
});

function onActionClick(e: OnActionClickParams<PushProxyApi.PushProxyVO>) {
  switch (e.code) {
    case 'delete': {
      onDelete(e.row);
      break;
...
function onEdit(row: PushProxyApi.PushProxyVO) {
  if (!hasAccessByCodes(['Media:PushProxy:Edit'])) {
    message.error('您没有编辑推流代理的权限');
    return;
  }
  formDrawerApi.setData(row).open();
}

function onDelete(row: PushProxyApi.PushProxyVO) {
  if (!hasAccessByCodes(['Media:PushProxy:Edit'])) {
    message.error('您没有删除推流代理的权限');
    return;
  }
...
function onDetail(row: PushProxyApi.PushProxyVO) {
  if (!hasAccessByCodes(['Media:PushProxy:View'])) {
    message.error('您没有查看推流代理详情的权限');
    return;
  }
...
async function onPlay(row: PushProxyApi.PushProxyVO) {
  if (!hasAccessByCodes(['Media:PushProxy:View'])) {
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('media.pushProxy.list')">
